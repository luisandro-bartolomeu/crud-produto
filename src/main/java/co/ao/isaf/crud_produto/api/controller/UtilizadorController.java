...
public class UtilizadorController {
    ...
    @GetMapping
    public ResponseEntity<List<Utilizador>> listarUtilizador() {
    ...
    @PostMapping("/login")
    public ResponseEntity<?> login(@RequestBody @Valid Utilizador u){
        return ResponseEntity.ok( utilizadorService.Login(u));
    }

    @Operation(summary = "Criar Utilizador")
    @PostMapping("/register")
    public ResponseEntity<Utilizador> criarUtilizador(@Valid @RequestBody Utilizador utilizador){
        var utilizador2 = utilizadorService.criaUtilizador(utilizador);
        return ResponseEntity.status(HttpStatus.CREATED).body(utilizador2);
    }
